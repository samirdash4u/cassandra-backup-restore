job "cassandra-backup-delete" {
	...
	task "cassandra-node1" {
		...
		template {
			data = <<EOH
				...
				ACTION="delete"
				BACKUP_COUNT="{{key "service/cassandra-backup-restore/backup_count"}}"
	...
	task "cassandra-node2" {
		driver = "docker"

		constraint {
			attribute = "${attr.unique.network.ip-address}"
			value = "192.168.13.225"
		}
		artifact {
			source = "https://rhel-224.test:8090/cassandra-backup_1.0.tar"
		}
